...
Generates a database file.
    ...
    usage: cxxtags [-p, --partial] [-e exclude_list] [-o output_file] input_file [compiler_arguments]

* -p, --partial  
  * Only partial part of database is built. This option helps save disk space. File local information is not generated. To get full part of information you can rebuild later by using *cxxtags_db_manager rebuild* command.
* -e  
  * Specify directories that contains source/header files that you don't want to tag. Directories are separated by ':' like "/usr/include:/usr/local/include".
* -o  
...
Merges some database files generated by **cxxtags** to a directory. 
    ...
    usage: cxxtags_db_manager add     database_dir input_files [...]
         :                    rebuild database_dir src_file_name

* command mode
  * add: Add specified database files to a database set.
  * rebuild: Rebuild a scpeifiled database file.
* -f, --force  
...
Once you generate a database file, build options you specified at build process are registered to a database so that
you can rebuild *cxxtags_db_manager* command without specifying them.
...
* step 2: make pseudo compiler commands
    * edit /path/to/cxxtags/installed/g++ as shown below. '-p' is recommended for saving disk space. When you become need to get
      file local information, rebuild database by cxxtags_db_manager. 
...
* step 4: finaly, merge database files
        ...
        $ cxxtags_db_manager add db `find ./ -name "*.o"`
