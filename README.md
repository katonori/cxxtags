...
* name: Specify the name of the token that you want to inspect.  
* query_type: Specify the type of query. Listed below are acceptable.  
    * decl: Query information about location where the token is **declared**.  
    * def: Query information about location where the token is **defined**.  
    * ref: Query information about all the locations where the token is **refered**  
* -f: Specify the file that contains the token.  
* -l: Specify the line number the token is located.  
* -c: Specify the column number the token is located.  
...
* step 2: make pseudo compiler commands
    * vim /path/to/cxxtags/installed/g++  
...
* step 3: then start building
    * env PATH=/path/to/cxxtags/installed make
